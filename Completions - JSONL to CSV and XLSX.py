    ...
    try:
        ...
        with open(input_file_path, 'r') as file:
            ...
            for line in data:
                ...
                prompt=line_data["prompt"].replace('\\n\\n###\\n\\n','')
                completion=line_data["completion"].replace('\\n','').lstrip()
                line_data_list=[prompt,completion]
...
n = len(sys.argv)
if n!=2:
    print("Invalid number of arguments\nCorrect Command Schema : python '.\Completions - JSONL to CSV and XLSX.py' [Input Filename]")
